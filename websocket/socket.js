...
var chat = io.on('connection', function (socket) {
            ...
            if (undefined != toSocket) {
                ...
                redisDB.get('online:' + data.toId, function (err, reply) {
                    if (!err && null != reply) {
                        ...
                        //发布一条消息
                        redisDB.publish('peerMsg:' + reply, JSON.stringify(data));

                    }
                    else {
                        //如果用户未登陆;将消息插入到未读消息列表;	
                        redisDB.lpush('unreadMsg:' + data.toId, JSON.stringify(data) , function (err, reply) {
                            if (!err && null != reply) {
    ...
    socket.on('group message', function (data) {
        if (undefined != data.toId) {
                ...
                if (!err && null != reply) {
                    ...
                    if (dataMap.size >= 1) {
                        dataMap.forEach(function (value, key, map) {
                            //发布一条消息
                            redisDB.publish('groupMsg:' + key,  JSON.stringify(data));
            ...
            //处理群未在线的用户消息
            //从左边入列表存储二进制数据;
            redisDB.lpush('groupMsg:' + data.toId,  JSON.stringify(data),   function (err, reply) {
                //只保存99条消息;如果超过截取后面99条;
                if (!err && null != reply) {
                    if (parseInt(reply) > 99) {
